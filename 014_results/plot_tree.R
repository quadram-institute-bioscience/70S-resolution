...
png ("operon%02d.png", width = 6000, height= 10000, bg = "white");
...
#   sp_strain  <- name_table[,3]; names (sp_strain)  <- tre$tip.label; # gives strain (sample id) for each leaf
    ...
    legend("bottomleft", legend=majclad, bty="n", fill=mac, border=mac, cex=5, inset=0.1);
}
dev.off(); quit();
