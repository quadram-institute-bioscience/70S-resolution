...
for (i in 1:length(treefile)) {
    ...
    p <- p + geom_tiplab2(align=TRUE,linetype=1, linesize=0.1, size=1)  
    p <- p + ggplot2::scale_color_manual(values=color_palette) 
    p + ggplot2::ggtitle(fname)
    ggplot2::ggsave (paste(fname,"_ggcircular.png",sep=""), width = 10, height= 10, bg = "white");

    tre$edge.length <- log(tre$edge.length + 1.)
    color_palette  <- colorRampPalette(brewer.pal(9, "Dark2"))(length(groupInfo) + 1); ## or Dark2 or Set1
