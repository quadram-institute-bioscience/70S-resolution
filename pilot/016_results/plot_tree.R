...
minclad <- c("Escherichia", "Helicobacter", "Leptospira", "Neisseria", "Pseudomonas", "Salmonella")
...
for (i in 1:length(treefile)) {
    ...
    col1 <- rep("black",len);
    for (k in 1:len) {
        for (j in 1:length(minclad)) { # look for anything at the beginning except "_", followed by "_minclad[]".
            if ( grepl(paste("^",minclad[j],"_",sep=""),tre$tip.label[k]) ) col1[k] <- mic[j];
        }
...
groupInfo <- split(tre$tip.label, gsub("_\\w+","", tre$tip.label))
